...
已登录且已发布一个数字人的普通用户，可在 `/app` 用户首页建立 WebRTC，收看该形象的音视频，并发送文字或麦克风识别结果驱动口型。未发布时不能连接。

## 首页

- 无已发布形象：提示前往「我的资产」发布；不展示开始连接按钮。
- 有已发布形象：展示名称、开始/断开、远端 video/audio、文本输入与发送。
- 连接后可用麦克风：「开始说话」采集音频，经 `GET /api/asr` WebSocket 识别；「停止识别」后若文本非空，按 echo 自动 `POST /human`（与文字发送相同，`type=echo`，带本次 `sessionid`）。空结果或失败只在本页提示，不发送。未连接时麦克风不可用。没有 FunASR 服务器地址栏，也没有 2pass/online/offline 模式选择。
...
连接成功后，前端把用户输入 POST 到现有 `/human`，带 `sessionid`、`text` 与 `type=echo`。本规格不要求 `/human` 登录。麦克风识别成功后走同一路径。
