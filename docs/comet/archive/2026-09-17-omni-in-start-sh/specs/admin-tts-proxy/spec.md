...
数字人会话默认 TTS 为 `omnitts`（见 `omni-tts-serving`）。8010 进程本身仍不启动、不安装 Omni；由 `./start.sh` 在拉起 8010 之前按 `omni-tts-serving` 拉起独立 8091 进程。
...
合成成功时把上游音频字节原样返回（合适的 `Content-Type`）。JSON 列表原样或包一层 `data`，前端能渲染音色名即可。`POST /speech` 的 JSON 含 `task_type`：预设音色为 `CustomVoice`，已上传克隆音色为 `Base`。代理原样转发该字段。
...
- 看到连接状态（上游可达或不可达）
- 刷新音色列表
- 选择音色、输入文本、合成并播放或下载；预设音色 `task_type=CustomVoice`，已上传克隆音色 `task_type=Base`
- 上传克隆音频（multipart，字段与 Omni 管理页一致：`audio_sample`、`name`、`consent`、`ref_text`，可选 `speaker_description`）。上游须为 Base 检查点才会成功；CustomVoice 上的失败展示上游原因，不假装成功。
...
- 不在 8010 进程内嵌 vLLM
- 不要求单测连接真实 8091
- 不实现 EdgeTTS 自动回退（会话默认 TTS 由 `omni-tts-serving` 规定）
- 不把默认 8091 检查点改为 Base
