...
开始连接时，浏览器 `POST /api/v1/admin/offer`（须管理员 Cookie），body 含 SDP 与所选 `avatar`。若填写了参考音频或参考文本，可随 offer 带上 `refaudio` / `reftext`。成功后保存 `sessionid`，播放远端视频，并把该 `sessionid` 绑到当前管理员。断开则关闭 RTCPeerConnection，并解除该绑定。

未登录调用该接口 401；普通用户 403。同一管理员已有由本接口创建的会话时，拒绝新的一路（明确错误）。计入全局 `max_session`。
...
本接口创建的会话：上述驱动须同一管理员 Cookie。未连接时不能发送、打断或使用麦克风。
...
对本接口创建的会话，上述请求同样须同一管理员 Cookie。

## 与原站关系

原 `web/index.html` 保持可匿名使用，路径为 `/index.html`，仍走匿名 `POST /offer`。麦克风 ASR 已在本页，不再提供「打开 /index.html」入口。本页不使用 `/api/v1/me/offer`。
