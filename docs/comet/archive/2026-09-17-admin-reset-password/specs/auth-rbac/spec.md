...
- `admin`：可开户、绑定订阅、禁用与启用普通用户、删除普通用户、为普通用户设置新密码、使用管理员壳、查看与创建生成任务、使用 TTS 代理。
...
管理员专属：创建用户、列出全部用户、绑定/解绑任意用户的订阅、禁用与启用普通用户、删除普通用户、为普通用户设置新密码、列出全部形象元数据、生成任务 CRUD、TTS 代理。普通用户调用返回 403。
