...
- 通过 `start-omni.sh` 启动，**默认**模型 `Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice`，端口 8091。
- 声音克隆改用同一脚本、环境变量 `OMNI_MODEL=Qwen/Qwen3-TTS-12Hz-0.6B-Base`，并重启 8091。首次无本地缓存时设 `HF_HUB_OFFLINE=0` 下载。权重不入库。
- 一个 `vllm serve` 只挂一个检查点；CustomVoice 与 Base 不能同时在同一 8091 上。
- 两阶段 pipeline 的 `gpu_memory_utilization` 各约为 0.15，避免默认 0.3+0.3 在空闲占满 4090 导致数字人 OOM。Base 与 CustomVoice 同级，可先保持 0.15。
...
- 8091 运行 Base 时，管理页可上传克隆音色并试听；运行 CustomVoice 时上传克隆会失败并显示上游原因。

## 升级路径

- `0.6B-Base`：`OMNI_MODEL=Qwen/Qwen3-TTS-12Hz-0.6B-Base ./start-omni.sh`，显存可先保持两阶段 0.15。
- `1.7B-CustomVoice` 仍只改 serve 模型 ID 与显存分数，**不在本规格内部署**。
