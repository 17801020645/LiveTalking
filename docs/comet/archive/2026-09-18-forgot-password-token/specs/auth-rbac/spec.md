...
LiveTalking 提供基于 Cookie 的登录与两种角色。新资源接口位于 `/api/v1/`。原站演示连麦 `POST /offer` 保持匿名。Avatar 生成任务接口仅管理员可调用。`/app` 里由登录用户创建的连麦会话，其驱动接口须同一用户。未登录用户可用管理员发放的一次性令牌重设密码；本规格不包含邮箱或验证码。

## 角色

- `admin`：可开户、绑定订阅、禁用与启用普通用户、删除普通用户、为普通用户设置新密码、为普通用户发放密码重置令牌、使用管理员壳、查看与创建生成任务、使用 TTS 代理、用 `POST /api/v1/admin/offer` 演示连麦。
...
- `POST /api/v1/auth/forgot-password`：公开。请求体 `{ "username", "token", "new_password" }`。令牌有效则更新密码哈希、作废该令牌、删除该用户全部平台登录会话；不设置登录 Cookie。新密码少于 6 个字符被拒绝且哈希不变。用户名不存在、令牌错误、过期或已使用：统一失败文案，不暴露该用户名是否存在。已禁用用户改密成功后仍不能登录，启用后须用新密码。
- 密码使用单向哈希存储。
...
需要登录的路径前缀：`/api/v1/`（`/api/v1/auth/login` 与 `POST /api/v1/auth/forgot-password` 除外）、`/api/avatar/`。
...
管理员专属：创建用户、列出全部用户、绑定/解绑任意用户的订阅、禁用与启用普通用户、删除普通用户、为普通用户设置新密码、为普通用户发放密码重置令牌、列出全部形象元数据、生成任务 CRUD、TTS 代理、`POST /api/v1/admin/offer`。普通用户调用返回 403。
...
- 代人改密：`POST /api/v1/admin/users/{user_id}/password`。仅 `role=user`（活跃或已禁用均可）。请求体 `{ "password" }`，至少 6 个字符（与开户相同）。不能给当前登录账号改密。成功后更新密码哈希，并删除该用户全部平台登录会话；`status`、订阅与订单不变。对管理员或不存在的 id 返回明确失败（400 或 404）；密码过短则失败且哈希不变。已禁用用户改密后仍不能登录，启用后须用新密码。代人改密同时作废该用户未使用的重置令牌。
- 发放重置令牌：`POST /api/v1/admin/users/{user_id}/reset-token`。仅 `role=user`（活跃或已禁用均可）。不能给当前登录账号或管理员发令牌。成功后返回明文令牌一次，库中只存哈希；有效期 24 小时；新令牌作废该用户未使用的旧令牌。对管理员或不存在的 id 返回明确失败（400 或 404）。
- 管理后台用户表：活跃普通用户显示禁用按钮；已禁用普通用户显示启用按钮；普通用户行另有删除、改密与发令牌按钮。禁用、启用、删除经确认；改密经确认后输入新密码；发令牌经确认后展示一次明文令牌。管理员行不显示这些按钮。
