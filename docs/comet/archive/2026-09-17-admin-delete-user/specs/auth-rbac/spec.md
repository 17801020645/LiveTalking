...
- `admin`：可开户、绑定订阅、禁用与启用普通用户、删除普通用户、使用管理员壳、查看与创建生成任务、使用 TTS 代理。
...
管理员专属：创建用户、列出全部用户、绑定/解绑任意用户的订阅、禁用与启用普通用户、删除普通用户、列出全部形象元数据、生成任务 CRUD、TTS 代理。普通用户调用返回 403。管理员不能通过后台重置其他用户密码。
...
- 管理后台用户表：活跃普通用户显示禁用按钮；已禁用普通用户显示启用按钮；普通用户行另有删除按钮。点按均经确认。管理员行不显示这些按钮。
- 本规格不包含邮箱、验证码、代人改密。
